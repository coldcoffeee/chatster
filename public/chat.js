...
function showMessage(message, name, url, createdAt) {
  ...
  <div class="bg-gray-800 rounded text-start p-4 my-3 mx-3">
    ...
    <div class="bg-gray-800 w-72 h-72 relative ${url ? "" : "hidden"} my-1">
        <img src="${url}" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); max-width: 100%; max-height: 100%;">
    </div>
      ${message ? message : ""}
...
async function sendMessage() {
  try {
    ...
    const res = await axios.post(host + "/chat/saveMessage", formData, config);
